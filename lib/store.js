...
async function createTask({ title, description, assigneeId, assigneeName, priority, dueDate, createdBy, status, sheetRow, requestDate }) {
  const id = generateId();
  const { rows } = await sql`
    INSERT INTO tasks (id, title, description, status, assignee_id, assignee_name, progress, priority, due_date, created_by, sheet_row, request_date)
    VALUES (${id}, ${title}, ${description || ''}, ${status || '대기'}, ${assigneeId || null}, ${assigneeName || null}, 0, ${priority || 'mid'}, ${dueDate || null}, ${createdBy || null}, ${sheetRow || null}, ${requestDate || null})
...
async function getTaskBySheetRow(sheetRow) {
  const { rows } = await sql`
    SELECT id, title, description, status,
           assignee_id AS "assigneeId",
           assignee_name AS "assigneeName",
